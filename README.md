...
the non-specific ones, which are also the ones whose single-site cancer risk
never crosses 3%. That overlap is what closes the argument, and it is computed,
not asserted: the intersection is derived in `verification/check_sources.py`.
...
rather than redrawing it: where a source figure exists and its licence permits
...
- **Virdee, Oke, Bankhead, et al. (2025)**, *JMIR Cancer* 11:e70275 — reviewed 7 cancer risk models built on blood-test trends. Across 14 external validations, **one** assessed calibration; all but one study scored high risk of bias in the analysis domain.
